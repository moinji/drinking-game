...
import { femalePreferences } from '../data/gameData'
...
const currentQuestion = computed(() => femalePreferences[currentIndex.value])
const progress = computed(() => Math.round((currentIndex.value / femalePreferences.length) * 100))
...
const selectChoice = (choice) => {
  ...
  setTimeout(() => {
    if (currentIndex.value < femalePreferences.length - 1) {
      currentIndex.value++
    } else {
...
<template>
  <div class="game-container">
    <h2 class="game-title">여자 취향 선택</h2>

    <div v-if="!isFinished" class="question-section">
      <div class="progress-bar">
        <div class="progress-fill" :style="{ width: progress + '%' }"></div>
      </div>
      <div class="progress-text">{{ currentIndex + 1 }} / {{ femalePreferences.length }}</div>
